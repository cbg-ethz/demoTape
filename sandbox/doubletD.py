...
class doubletFinder():
    ...
    def __init__(self, df_total, df_alt, delta, beta, mu_hetero, mu_hom, alpha_fp, alpha_fn, missing=False, verbose=True, binom=False, precision=None):
        ...
        print(f"alpha_fn = {self.alpha_fn:.4f}")
        print(f"alpha_fp = {self.alpha_fp:.4f}")
        ...
        with open(outputFile, 'w') as output:
            ...
            for cell in self.cells:
                ...
                if self.doublet_result[cell] == 'doublet':
                    dbt += 1
        print(f'\nEstimated doublet rate: {dbt/len(self.cells):.3f}\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inputTotal", type=str,
        help="csv file with a table of total read counts for each position in each cell")
    parser.add_argument("--inputAlternate", type=str,
        help="csv file with a table of alternate read counts for each position in each cell")
    parser.add_argument("--delta", type=float, default=0.1,
        help="expected doublet rate [0.1]")
    parser.add_argument("--beta", type=float, default=0.05,
        help="allelic dropout (ADO) rate [0.05]")
    parser.add_argument("--mu_hetero", type=float,
        help="heterozygous mutation rate [None]")
    parser.add_argument("--mu_hom", type=float,
        help="homozygous mutation rate [None]")
    parser.add_argument("--alpha_fp", type=float,
        help="copy false positive error rate [None]")
    parser.add_argument("--alpha_fn", type=float,
        help="copy false negative error rate [None]")
