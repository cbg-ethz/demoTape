...
def get_SNParray_profiles(in_files):
    for in_file in in_files:
        df = pd.read_csv(in_file, comment='#', sep='\t', header=None)
        df.columns = VCF_COLS
        ...
        new_gt = pd.DataFrame(df['sample'].apply(BAF_to_gt))
        new_gt.index = idx_str
        new_gt = new_gt.reset_index().drop_duplicates().set_index('index')

        sample_name = os.path.basename(in_file).split("_")[0]

        try:
            if sample_name in SID2PAPER:
                SID = sample_name
            else:
                SID = PID2SID[sample_name]
        except KeyError:
            sample_name = f'{sample_name}_SNParray'
        else:
            try:
                sample_name = f'{SID2PAPER[SID]}_SNParray'
            except KeyError:
                sample_name = f'{SID}_SNParray'
...
def main(args):
    if args.profiles:
        SNP_df = get_scRNA_profiles(args.profiles)
        label = 'scRNA-seq'
    elif args.SNParray:
        SNP_df = get_SNParray_profiles(args.SNParray)
        label = 'SNP array'
    else:
        raise IOError('Requires either scRNA profiles or SNP array data')

    # SNP_df.index = ["_".join(i.split("_")[:2]) for i in SNP_df.index]
    ...
    for i, in_file in enumerate(args.input):
        if re.search('cells_variants.(\d+).csv', in_file):
            ...
            name = f'{pat}_DNA'
        elif re.search('\+wlSNParray.(\d+).csv', in_file):
            cluster = re.search('\+wlSNParray.(\d+).csv', in_file).group(1)
            name = f'{Cluster2PAPER[cluster]}_DNA'
        else:
            name = f'{i}_DNA'
    if not args.outfile:
        ...
        # df_new.index = ["_".join(i.split("_")[:2]) for i in df_new.index]
    ...
    dists = []
    for name in names:
        dists.append(np.apply_along_axis(dist_nan, 1, df[SNP_df.columns].values.T,
            df[name].values.T))

    dist_df = pd.DataFrame(dists,index=names, columns=SNP_df.columns)

    assign_clusters(dist_df, args.assignment-output)
    print(f'\n{dist_df}\n')
...
def parse_args():
    ...
    parser.add_argument('-a', '--SNParray', type=str, nargs='+',
        help='Input SNP array mutations in vcf format.')
    parser.add_argument('-ao', '--assinment-output', type=str,
        help='Output yaml-file in which the automated sample assigment is saved.')
